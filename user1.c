...
int main(int argc,char* argv[]){
  ...
  while((read_bytes=read(fd,buffer,BUFSIZE))>0){
    ...
    while((sent_bytes=k_sendto(sockfd,buffer,read_bytes,0,(struct sockaddr*)&serveraddr,sizeof(serveraddr)))<0 && errno==ENOBUFS){
      sleep(1);
    }
    if(sent_bytes<0){
    ...
    printf("[+] Sent %d bytes, packet sent=%d!!!\n",sent_bytes,cnt);
    cnt++;
  }
  buffer[0]='#';
  while((sent_bytes=k_sendto(sockfd,buffer,1,0,(struct sockaddr*)&serveraddr,sizeof(serveraddr)))<0 && errno==ENOBUFS){
    sleep(1);
  }
  if(sent_bytes<0){
    perror("[-] Error in sending");
